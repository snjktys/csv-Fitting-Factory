...
TEXTS = {
    ...
    "en": {
        ...
        "high_degree_question": "Polynomial degrees 6–10 may overfit or oscillate near the edges. Continue?",
...
MODEL_LABELS = {
    "zh": {"polynomial": "多项式", "exponential": "指数", "sine": "正弦", "logistic": "Logistic"},
    "en": {"polynomial": "Polynomial", "exponential": "Exponential", "sine": "Sine", "logistic": "Logistic"},
}
ERROR_LABELS = {
...
PARAMETER_DESCRIPTIONS = {
    "en": {
        "exponential": ("scale (offset from baseline at x=0)", "growth or decay rate", "vertical offset (baseline)"),
        "sine": ("amplitude", "frequency (cycles per x unit)", "phase (radians)", "vertical offset"),
        "logistic": ("difference between plateaus", "growth/decay rate (+ rising, − falling)", "x value at the midpoint", "lower-plateau baseline"),
    },
}
...
_EXACT_MESSAGES = {
    "高阶多项式可能过拟合或在数据边缘产生振荡。": "High-degree polynomials may overfit or oscillate near the data edges.",
    "参数协方差无法可靠估计，请谨慎解释参数误差。": "Parameter covariance could not be estimated reliably; interpret parameter errors with caution.",
    "指数初值采用了安全默认值，必要时请手动调整。": "Safe default exponential initial values were used; adjust them manually if needed.",
    ...
    "清洗后没有有效数值，请更换数据列或修正 CSV。": "No valid numeric rows remain. Select other columns or correct the CSV file.",
    ...
    "CSV 格式无法解析，请检查分隔符和每行列数。": "The CSV format could not be parsed. Check the delimiter and column count.",
    "没有读取该文件的权限，请关闭占用程序或更换文件。": "The file cannot be read. Close any program using it or choose another file.",
    ...
    "X 间隔不均匀，正弦频率使用 1/X范围 作为初值。": "X spacing is uneven; 1/X range was used as the initial sine frequency.",
    "有效采样间隔较少，正弦频率使用安全默认值。": "Too few valid sampling intervals; a safe default sine frequency was used.",
    "Y 数据变化太小，不适合 Logistic 拟合。": "Y varies too little for logistic fitting.",
    "X 数据没有范围，无法估计 Logistic 初值。": "X has no range, so logistic initial values cannot be estimated.",
    "所有初始参数都必须是数字。": "All initial parameters must be numeric.",
    "所有初始参数都必须是有限数值。": "All initial parameters must be finite.",
    "拟合参数会导致模型计算溢出，请调整初值或数据尺度。": "The fitted parameters overflow the model. Adjust the initial values or data scale.",
    "拟合结果包含无效数值，请调整初值或数据尺度。": "The fitting result contains invalid values. Adjust the initial values or data scale.",
    "蒙特卡洛重采样次数必须大于 0。": "The Monte Carlo iteration count must be greater than zero.",
    "蒙特卡洛重采样全部失败，请检查数据或拟合参数。": "All Monte Carlo resamples failed. Check the data or fitting parameters.",
...
def translate_message(message: str, language: str) -> str:
    ...
    replacements = (
        ...
        (r"有效数据点只有 (\d+) 个，必须多于模型的 (\d+) 个参数。", r"Only \1 valid data points remain; more than \2 model parameters are required."),
        (r"不同的 X 只有 (\d+) 个，至少需要 (\d+) 个。", r"Only \1 distinct X values remain; at least \2 are required."),
    )
